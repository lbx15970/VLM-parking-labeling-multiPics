...
用于对比多模态模型（Doubao Seed 1.8、Seed 2.0 Pro、Qwen-VL-Max、Qwen3-VL）在停车场目标检测/框选任务上的效果，并输出：
...
│   └── images/                 # 本地测试图片 (00001~00020.jpg)
├── outputs/
│   ├── csv/                    # CSV 汇总结果
│   ├── bboxes/                 # 详细 bbox 坐标与 IoU 记录 (txt)
...
使用 `test_prompt_comparison.py`，支持 Seed 1.8、Seed 2.0 Pro、Qwen-VL-Max、Qwen3-VL 的多维度对比。
...
针对 Qwen3-VL 模型输出的 bounding box 存在「长边 1000 归一化」特性（而非宽高独立归一化），
...
- **2026-03-07**
  ...
  - 数据源从 2 张 URL 图片 + XML 标注迁移到 20 张本地图片 (00001~00020.jpg) + JSON 标注
  - 新增 `prompt优化v3.md`（针对停车位编号识别优化）
  - 修复 Qwen3-VL 归一化坐标解析导致的标注框漂移问题
  - 输出新增 `bboxes/` 目录记录详细坐标与 IoU
