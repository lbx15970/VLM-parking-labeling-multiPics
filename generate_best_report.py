...
# 按模型和图片分组，找出每组 IoU 最高的那一次
best_runs = defaultdict(dict)  # key: (model, image) -> row
...
for row in data:
    ...
    if key not in best_runs:
        ...
        if row['IoU'] > best_runs[key]['IoU']:
            best_runs[key] = row

# 转换为列表
filtered_data = list(best_runs.values())
...
# 生成报告
report = []
report.append(f"# Qwen3.5-Plus vs Doubao-Seed-2.0-Pro —— Prompt 优化 v4 测试对比报告 (Best Run)")
report.append(f"")
report.append(f"## 概述")
report.append(f"")
report.append(f"- **对比对象**: Qwen3.5-Plus vs Doubao-Seed-2.0-Pro")
report.append(f"- **测试输入**: {len(unique_images)} 张图片，**每张图片每模型选取 IoU 最高的 1 次**（从 3 次运行中挑选），5 线程并发")
...
report.append(f"---")
report.append(f"")
report.append(f"## 分图片详细对比 (Best Run)")
report.append(f"")
...
for img in sorted_images:
    for model in ['qwen3.5-plus', 'seed2.0-pro']:
        if img not in images or model not in images[img]:
            continue
        
        row = images[img][model][0]  # 只有一个
        ...
        fp = row['FP']
        fn = row['FN']
        gt_count = int(row['GT数'])
        
        model_name = "Qwen3.5" if model == 'qwen3.5-plus' else "Seed-2.0"
        report.append(f"| {img} | {model_name} | {precision:.1%} | {recall:.1%} | {iou:.3f} | {tp:.1f} | {fp:.1f} | {fn:.1f} | {gt_count} |")

# 输出到文件
output_path = f"docs/qwen3.5_vs_seed2.0_prompt_v4_test_report_best_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
